...
type Tx = {
  ...
  activity: string;
  activityLevels: { value: string; label: string; multiplier: number }[];
  calculate: string;
  recalculate: string;
  bmrLabel: string;
  bmrNote: string;
...
export const tdeeT: Record<Lang, Tx> = {
  id: {
    ...
    calculate: "Hitung Kebutuhan Kalori",
    recalculate: "Hitung Ulang",
    bmrLabel: "BMR (Kalori Dasar)",
    bmrNote: "Kalori yang dibakar tubuh saat istirahat total",
    maintenanceLabel: "Kalori Maintenance",
    maintenanceNote: "Untuk menjaga berat badan saat ini",
    cutLabel: "Target Defisit (Turun BB)",
    cutNote: "−500 kcal/hari → turun ~0.5 kg/minggu",
    bulkLabel: "Target Surplus (Naik BB)",
    bulkNote: "+300 kcal/hari → lean bulk",
    macrosTitle: "Estimasi Makro Harian (Maintenance)",
    protein: "Protein",
    carbs: "Karbohidrat",
    fat: "Lemak",
  },
  en: {
    ...
    calculate: "Calculate My Calories",
    recalculate: "Recalculate",
    bmrLabel: "BMR (Basal Metabolic Rate)",
    bmrNote: "Calories burned at complete rest",
    maintenanceLabel: "Maintenance Calories",
    maintenanceNote: "To maintain your current weight",
    cutLabel: "Cut Target (Lose Weight)",
    cutNote: "−500 kcal/day → lose ~0.5 kg/week",
    bulkLabel: "Bulk Target (Gain Weight)",
    bulkNote: "+300 kcal/day → lean bulk",
    macrosTitle: "Estimated Daily Macros (Maintenance)",
    protein: "Protein",
    carbs: "Carbs",
    fat: "Fat",
  },
...
export default function TdeeCalculator({ lang }: { lang: Lang }) {
  ...
  function calculate(e: React.FormEvent) {
    ...
    const multiplier =
      tx.activityLevels.find((l) => l.value === activity)?.multiplier ?? 1.55;
    const tdee = bmr * multiplier;

    // Macros for maintenance: protein 2g/kg, fat 25%, rest carbs
      ...
      {result && (
        <div className="mt-6 space-y-3">
          ...
          <div className="rounded-3xl border-2 border-emerald-200 bg-emerald-50 p-5">
            <p className="text-xs font-semibold uppercase tracking-widest text-emerald-500 mb-0.5">
              {tx.maintenanceLabel}
            </p>
            <p className="text-6xl font-black text-emerald-700">
              {result.tdee.toLocaleString()}
              <span className="text-xl font-normal text-emerald-400 ml-1">kcal</span>
            </p>
            <p className="text-xs text-emerald-500 mt-1">{tx.maintenanceNote}</p>
          </div>

          {/* Cut + Bulk side by side */}
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border-2 border-blue-100 bg-blue-50 p-4">
              ...
              <p className="text-2xl font-black text-blue-700">
                {result.cut.toLocaleString()}
                <span className="text-sm font-normal text-blue-400 ml-1">kcal</span>
              </p>
            ...
            <div className="rounded-2xl border-2 border-orange-100 bg-orange-50 p-4">
              ...
              <p className="text-2xl font-black text-orange-600">
                {result.bulk.toLocaleString()}
                <span className="text-sm font-normal text-orange-400 ml-1">kcal</span>
              </p>
          ...
          <div className="rounded-2xl border-2 border-emerald-100 bg-white p-4">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              {[
                { label: tx.protein, value: result.macros.protein, unit: "g", color: "text-blue-600" },
                { label: tx.carbs, value: result.macros.carbs, unit: "g", color: "text-orange-500" },
                { label: tx.fat, value: result.macros.fat, unit: "g", color: "text-yellow-500" },
              ].map((m) => (
